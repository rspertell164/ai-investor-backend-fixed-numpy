...
def get_news(symbol: str):
    ...
    try:
        return [
            {"headline": f"{symbol.upper()} set to report earnings next week", "sentiment": "neutral"},
            {"headline": f"{symbol.upper()} surges on new product launch", "sentiment": "positive"},
            {"headline": f"{symbol.upper()} faces regulatory scrutiny", "sentiment": "negative"}
        ]
    except Exception as e:
        return [{"headline": f"Error retrieving news: {str(e)}", "sentiment": "neutral"}]
